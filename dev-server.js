...
app.get('/api/b2-upload', async (req, res) => {
  ...
  try {
    ...
    // CRITICAL: Proper URL encoding for B2
    const bucketEncoded = encodeURIComponent(bucketName);
    const keyEncoded = key.split('/').map(encodeURIComponent).join('/');
    const downloadUrl = `${authData.downloadUrl}/file/${bucketEncoded}/${keyEncoded}`;
    ...
    if (!downloadResponse.ok) {
      ...
      return res.status(downloadResponse.status).json({ error: `Download failed (${downloadResponse.status})` });
    }
